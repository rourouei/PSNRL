        ...
        for rafd_dir in rafd_dirs:
            for i in subject:
                ...
                for file in os.listdir(rafd_dir):
                    if file.find(si, 7, 12) != -1 and file.find('neutral') == -1:
                        expr.append(os.path.join(rafd_dir, file))
                    if file.find(si, 7, 12) != -1 and file.find('neutral') != -1:
                        neu.append(os.path.join(rafd_dir, file))
                for e in expr:
                    for n in neu:
                        pair = [e, n]
                        pairs.append(pair)
...
class rafdPairLoaderAlignTest(data.Dataset):
    def __init__(self, subject):
        super(rafdPairLoaderAlignTest, self).__init__()
        pairs = []
        rafd_dirs = [rafd + '/RaFD_center']
        for rafd_dir in rafd_dirs:
            for i in subject:
                if i < 10:
                    si = '0' + str(i)
...
# dataset = rafdPairLoaderAlign([i for i in range(1, 74)])
...
dirCheckpoints = dirRoot + '/code/PSNRL/checkpoints/idnet_rafd'
...
def train_idnet():
    ...
    for epoch in range(args.nepoch):
        dataset = rafdPairLoaderAlign(train_split[0])
        print('# size of the current (sub)dataset is %d' % len(dataset))
...
def test_idnet(iteration_count):
    dataset = rafdPairLoaderAlignTest(test_split[0])
    print('# size of the current (sub)dataset is %d' % len(dataset))
