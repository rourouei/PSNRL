...
class ouluPairLoaderAlignTest(data.Dataset):
    def __init__(self, subject):
        ...
        pairs = []
        oulu_dirs = ['/home/njuciairs/zmy/data/Oulu_aug/Oulu_center']
        for dir in oulu_dirs:
            for i in subject:
                if i < 10:
                    dirpath = dir + '/P00' + str(i)
...
dirCheckpoints = dirRoot + '/code/PSNRL/checkpoints/idnet_oulu_split'
dirImageoutput = dirCheckpoints + '/train_oulu_img'
dirTestingoutput = dirCheckpoints + '/test_oulu_img'
dirModel = dirCheckpoints + '/model'
load_encoder = dirRoot + '/code/DTNet1/checkpoints/idnet_rafd/model/encoder_epoch_50.pth'
load_decoder = dirRoot + '/code/DTNet1/checkpoints/idnet_rafd/model/decoder_epoch_50.pth'
...
def write_losses(klds, fpls, losses, train=True):
    if train == True:
        kld_file = dirCheckpoints + '/kld_loss_train.txt'
        fpl_file = dirCheckpoints + '/fpl_loss_train.txt'
        loss_file = dirCheckpoints + '/total_loss_train.txt'
    else:
        kld_file = dirCheckpoints + '/kld_loss_test.txt'
        fpl_file = dirCheckpoints + '/fpl_loss_test.txt'
        loss_file = dirCheckpoints + '/total_loss_test.txt'
    data_write_csv(kld_file, klds)
    data_write_csv(fpl_file, fpls)
    data_write_csv(loss_file, losses)
...
beta = 0.2
fpls, klds, totals = [], [], []
fpls_t, klds_t, totals_t = [], [], []
def train_idnet():
    iteration_count = 0
    for epoch in range(args.nepoch):
        dataset = ouluPairLoaderAlign(train_split[0])
        print('# size of the current (sub)dataset is %d' % len(dataset))
        ...
        for i, batch in enumerate(dataloader):
            ...
            if iteration_count % args.log_interval == 0:
                ...
                fpls.append(fpl_loss.item())
                klds.append(kld_loss.item())
                totals.append(loss.item())
                write_losses(klds, fpls, totals, True)
...
def test_idnet(iteration_count):
    dataset = ouluPairLoaderAlignTest(test_split[0])
    print('# size of the current (sub)dataset is %d' % len(dataset))
    dataloader1 = torch.utils.data.DataLoader(dataset, batch_size=batch_size, shuffle=True,
                                             num_workers=4)
    encoder.eval()
    decoder.eval()
    fpl_sum, kld_sum, loss_sum = 0, 0, 0
    for j, batch in enumerate(dataloader1):
        with torch.no_grad():
            source = batch[0].cuda()
            target = batch[1].cuda()
            z, mu, logvar = encoder(source)
        ...
        _source = transform_convert(source)
        _target = transform_convert(target)
        print('[{}/{}] KLD: {:.4f} FPL: {:.4f} TOTAL: {:.4f}'.format(i, len(dataloader1),
            kld_loss, fpl_loss, loss))
        # if iteration_count % args.img_interval == 0:
        if True:
            visualizeAsImages(_source.data.clone(),
                              dirTestingoutput,
                              filename='iter_' + str(iteration_count) +'_' + str(j) + '_source_', n_sample=64, nrow=8,
                              normalize=False)
            visualizeAsImages(_target.data.clone(),
                              dirTestingoutput,
                              filename='iter_' + str(iteration_count) + '_' + str(j) + '_target_', n_sample=64, nrow=8,
                              normalize=False)
            visualizeAsImages(_gene.data.clone(),
                              dirTestingoutput,
                              filename='iter_' + str(iteration_count) + '_' + str(j) +  '_gene_', n_sample=64, nrow=8,
                              normalize=False)
        ...
        kld_sum += kld_loss
        loss_sum += loss
    fpls_t.append((fpl_loss/(j+1)).item())
    klds_t.append((kld_loss/(j+1)).item())
    totals_t.append((loss/(j+1)).item())
    # print('KLD: {:.4f} FPL: {:.4f} TOTAL: {:.4f}'.format(kld_loss/(j+1), fpl_loss/(j+1), loss/(j+1)))
    write_losses(klds_t, fpls_t, totals_t, False)
    encoder.train()
    decoder.train()
