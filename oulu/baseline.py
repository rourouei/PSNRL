...
oulu_dirs = [oulu + '/Oulu_center', oulu+'/Oulu_rup',oulu+'/Oulu_rdown', oulu+'/oulu_lup', oulu+'/oulu_ldown'
             ,oulu+'/Oulu_rupf',oulu+'/Oulu_rdownf', oulu+'/oulu_lupf', oulu+'/oulu_ldownf']
...
dirCheckpoints = dirRoot + '/code/PSNRL/checkpoints/baseline_oulu'
...
parameters2 = list(encoderF.parameters()) + list(classifier.parameters())
        ...
        for i, batch in enumerate(dataloader):
            ...
            if iteration_count % args.log_interval == 0:
                print('[{}/{}][{}/{}] LOSS: {:.4f} Acc: {}'.format(
                    epoch, args.nepoch, i, len(dataloader), loss, accuracyComp(out, label)))
            if i % 100 == 0:
                ...
                torch.save(encoderF.state_dict(), '{}/encoderF_epoch_{}.pth'.format(dirModel, iteration_count))
                torch.save(classifier.state_dict(), '{}/classifier_epoch_{}.pth'.format(dirModel, iteration_count))
...
def test_classifier(iteration_count, k):
    ...
    for i, batch in enumerate(dataloader):
        with torch.no_grad():
            ...
            print('[{}/{}] LOSS: {:.4f} Acc: {}'.format(
                i, len(dataloader), loss, accuracyComp(out, label)))
...
label_path = dirCheckpoints + '/label1.txt'
pred_path = dirCheckpoints + '/pred1.txt'
