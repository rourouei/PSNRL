...
dirCheckpoints = dirRoot + '/code/PSNRL/checkpoints/dnet_oulu'
...
optimizer1 = optim.Adam(parameters1, lr=args.lr, betas=(args.beta1, 0.999), weight_decay=1e-4)
alpha = 1.0
beta = 0.2
def train_idnet(k):
    iteration_count = 0
...
optimizer3 = optim.Adam(parameters3, lr=args.lr, betas=(args.beta1, 0.999), weight_decay=1e-4)
def train_facenet():
    iteration_count = 0
    for epoch in range(4):
...
optimizer2 = optim.Adam(parameters2, lr=args.lr, betas=(args.beta1, 0.999), weight_decay=1e-4)
